...
function pbms(endpointUrl, requestId, txnId, operatorId, mobile_no, ConsumerNumber, amount, panel_id, circle_id) {
    return new Promise((resolve, reject) => {
        ...
        // Log the base URL (mask sensitive info)
        const baseUrl = endpointUrl.split('?')[0];
        console.log('=== PBMS API CALL ===');
        ...
        // Log full API URL (be careful with sensitive data)
        console.log('Full API URL (partial):', apiUrl.substring(0, 100) + '...');

        axios.get(apiUrl)
            .then((response) => {
                console.log('=== PBMS API RESPONSE ===');
            ...
            .catch((error) => {
                ...
                if (error.response) {
                    ...
                    reject({ result: error.response.data, panel_id: panel_id });
                } else {
                    reject(error);
                }
            });
