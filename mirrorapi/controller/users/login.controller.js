...
class Login {
  ...
  async login(req, res) {
    ...
    try {
      ...
      // ------------------------------
      let userRow = await this.db.user.findOne({ where: { mobile } });
      ...
      if (userRow) {
        if (email && userRow.email !== email) {
          return res.status(400).json(
            utility.DataEncrypt(JSON.stringify({
              status: 400,
              message: "Email does not match with this mobile number",
              data: []
            }))
          );
        }

        userData = await this.prepareUserData(userRow.id);
        otpCategory = "Login"; // OTP type after login
      } else {
        // ---------------------------------------------------
        ...
        if (!email) {
          return res.status(400).json(
            utility.DataEncrypt(JSON.stringify({
              status: 400,
              message: "Email is required for new registration",
              data: []
            }))
          );
        }

        let referredByMobile = null;
        if (referred_by) {
          const referralUser = await this.db.user.findOne({ where: { mobile: referred_by } });
          if (referralUser) {
            referredByMobile = referralUser.mobile;
          }
        }
        ...
        const registrationData = {
          ...
          email,
          username: mobile,
          first_name: mobile, // use mobile instead of first_name
          last_name: "",      // empty
          referred_by: referredByMobile,
          status: 1,
          created_on: currentTimestamp
        };

        const newUser = await this.db.user.create(registrationData);
        userData = await this.prepareUserData(newUser.id);
        otpCategory = "Register"; // OTP type after registration

        // Send welcome messages
        // try {
        //   const emailMsg = await emailUtility.registerEmailMessage(userData.first_name, userData.last_name, userData.mobile);
        //   const whatsappMsg = await whatsappUtility.registerWhatsappMessage(userData.first_name, userData.last_name, userData.mobile);
        //   messagingService.sendMessage(userData.mobile, whatsappMsg, null, userData.email, "Welcome to Boltpe", emailMsg);
        // } catch (msgErr) {
      ...
      // ------------------------------
      // SEND OTP AFTER LOGIN / REGISTER
      // ------------------------------
      try {
        const otpMobileUrl = process.env.OTP_URL + "/get-mobile-otp";
        console.log("Sending OTP to Mobile URL:", otpMobileUrl);

        // Prepare payload
        const mobileOtpPayload = {
          mode: "API",
          type: "Mobile",
        ...
        // Encrypt payload
        const encryptedPayload = { encReq: utility.DataEncrypt(JSON.stringify(mobileOtpPayload)) };

        // Send Mobile OTP
        await axios.post(otpMobileUrl, encryptedPayload);
        console.log("OTP sent to mobile");

        // Email OTP
        if (email) {
          const otpEmailUrl = process.env.OTP_URL + "/get-email-otp";
          console.log("Sending OTP to Email URL:", otpEmailUrl);

          await axios.post(otpEmailUrl, {
            mode: "API",
            type: "email",
            category: otpCategory,
            email
      ...
      return res.status(200).json(
        utility.DataEncrypt(JSON.stringify({
          ...
          refreshToken,
          otpCategory, // added
          message: userRow ? "Login successful (OTP sent)" : "User registered and login successful (OTP sent)",
          data: userData
        }))
      );
  ...
  async prepareUserData(userId) {
    ...
    const details = await this.db.userDetails.findOne({ where: { id: userId } });
