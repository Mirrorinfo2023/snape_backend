...
class RechargeReport {
  ...
  async recharge(req, res) {
    ...
    const { from_date, to_date } = req;
        ...
        console.log("req from recharge ", req);
    ...
    try {
        ...
        const results = await this.db.recharge.findAll({
            where: {
                created_on: {
        ...
        console.log("recharge data are ", results);

        if (results && results.length > 0) {
            return res.status(200).json({
                status: 200,
                message: 'Successfully all record found1',
                data: results
            });
        }
