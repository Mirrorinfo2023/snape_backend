...
class BillPaymentReport {
  ...
  async bill_payment(req, res) {
    ...
    try {
        ...
        const results=await this.db.billPaymentReport.findAll({
          where: {
            created_on: {
  ...
  async billPaymentHoldList(req, res) 
  {
    ...
    try {
        ...
        if(searchkey !=null && searchkey!='')
        {
          searchParam = {
            [Op.or]: [
              { 'payment_status': { [Op.like]: '%' + searchkey + '%' } },
        ...
        const getRechargeData = await paginate(this.db.billPaymentReport, {
          whereClause,
          order: [['id', 'DESC']],
          page: page?page:1
        });
...
module.exports = new BillPaymentReport();