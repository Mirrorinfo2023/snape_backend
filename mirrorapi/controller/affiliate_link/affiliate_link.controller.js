...
class AffiliateLink {
  ...
  async getAffiliateLinksByTitle(req, res) {

    // 🔐 Decrypt incoming request
    const decryptedObject = utility.DataDecrypt(req.body.encReq);
    // const decryptedObject = (req.body);
    const { title } = decryptedObject;

    // 🔍 Required field validation
    const requiredKeys = Object.keys({ title });
    ...
    try {
      // 📌 Fetch data
      const data = await this.db.affiliateLink.getDataByTitle(title);
  ...
  //   const { title, link, category_id, subcategory, amount } = req.body;

  //   // Validate required fields
  //   const requiredKeys = ['title', 'link', 'category_id'];
  //   if (!requiredKeys.every(key => req[key] !== undefined && req[key] !== '')) {
  //     return res.status(400).json({
  //       status: 400,
  //       message: 'Required input data is missing or empty',
  //       columns: requiredKeys
  ...
  //     const uploadPath = 'uploads/affiliate_link/';
  ...
  //     logger.error(`Error in add Affiliate Link: ${error}`);
  ...
  async addAffiliateLink(filename, req, res) {
    ...
    // Destructure the incoming request
    const { title, subtitle, link, category_id, amount } = req.body;
    console.log("req.body ", req.body)

    // Validate required fields
    const requiredKeys = ['title', 'link'];
    if (!requiredKeys.every(key => req.body[key] !== undefined && req.body[key] !== '')) {
      return res.status(400).json({
        status: 400,
        message: 'Required input data is missing or empty',
        columns: requiredKeys
    ...
    try {
      ...
      const Data = {
        title, // This will be the category name
        subtitle, // This will be the subcategory
        link,
        category_id: category_id || null, // Optional now
        amount: amount || 0,
        image: uploadPath + filePath,
      };
  ...
  async getAffiliateLinksAdmin(req, res) {
    ...
    try {
      ...
      const affiliateList = await this.db.affiliateLink.getAffliateList(whereCondition);
      const meetingResult = [];
  ...
  async updateAffiliateLink(filename, req, res) {
    try {
      const { id, title, subtitle, category_id, link, status, amount, valid_date } = req.body;
      ...
      // Fetch existing record to preserve image if not updating
      const existingRecord = await this.db.affiliateLink.getAffiliateData(id);
      if (!existingRecord) {
      ...
      const updateData = {
        ...
        subtitle: subtitle || existingRecord.subtitle,
        category_id: category_id || existingRecord.category_id,
        link: link || existingRecord.link,
        status: status !== undefined ? status : existingRecord.status,
        amount: amount || existingRecord.amount,
        modified_on: modified_on
      };

      // Add valid_date if provided
      if (valid_date) {
        updateData.valid_date = valid_date;
      } else if (existingRecord.valid_date) {
        updateData.valid_date = existingRecord.valid_date;
      }

      // Handle image - use new file or keep existing
      if (filename === null) {
        updateData.image = existingRecord.image;
        console.log("updateData from else", updateData)

      } else {
        const path = 'uploads/affiliate_link/';
        updateData.image = path + filename;
        console.log("updateData from if", updateData)
      }
      ...
      if (updatedStatus && updatedStatus.error === 0) {
        return res.status(200).json({
          status: 200,
