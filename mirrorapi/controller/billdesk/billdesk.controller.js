...
class BillDesk {
    ...
    async billdesk_request(req,res) {  
      ...
      try
      {
  const redirect_url = `https://secure.snape.org.in/api/bill_payment/billdesk-response`;
      const ipAddress = "185.7.81.191";
        const {result, reqData} = await billdeskUtility.BillDeskRequest(amount, redirect_url, ipAddress);
        
console.log("🔁 BillDesk Response (Raw):", result);
      console.log("📤 BillDesk Request Payload Sent:", reqData);
