...
class Recharge {
  ...
  async recharge(req, res, ipAddress) {
    ...
    console.log('IP:', ipAddress, 'User ID:', req.encReq ? 'encrypted' : 'no encReq');
    
    const decryptedObject = utility.DataDecrypt(req.encReq);
    console.log('Decrypted:', { mobile: decryptedObject.mobile, amount: decryptedObject.amount, type: decryptedObject.type, cwallet: decryptedObject.cwallet });
    ...
    if (!requiredKeys.every(key => key in decryptedObject && decryptedObject[key] !== '' && decryptedObject[key] !== undefined)) {
        console.log('Missing fields:', requiredKeys.filter(key => !(key in decryptedObject) || decryptedObject[key] === '' || decryptedObject[key] === undefined));
        return res.status(400).json(utility.DataEncrypt(JSON.stringify({ status: 400, message: 'Required input data is missing or empty', columns: requiredKeys })));
    }
    ...
    try {
        let userId = user_id;
        const checkplan = await this.db.PlanPurchase.getAllPlanUser(userId);
        console.log('User plans:', checkplan.length > 0 ? checkplan : 'No plans');
        ...
        console.log('User type:', user_type, 'Max plan:', maxPlan);
        ...
        lastDay = utility.formatDate(lastDay);
        let walletbalance = await this.db.wallet.getWalletAmount(userId);
        console.log('Wallet balance:', walletbalance, 'Amount needed:', amount);
        ...
        let totalDayRecharge = await this.db.recharge.getRechargeDataForDay(userId, crdate);
        let totalRechargeOfaMonth = await this.db.recharge.getRechargeForMonth(userId, firstDay, lastDay);
        console.log('Daily recharges:', totalDayRecharge, 'Monthly recharges:', totalRechargeOfaMonth);
        ...
        if (lastRecharge) {
            ...
            const timeDifference = date - lastRecordTime;
            minutesDifference = timeDifference / (1000 * 60);
            console.log('Last recharge time diff (min):', minutesDifference);
        }

        if (minutesDifference != null && minutesDifference <= 1) {
            console.log('Duplicate recharge too soon');
        ...
        if (user_type != 'Prime' && totalRechargeOfaMonth >= 1) {
            console.log('Non-prime user monthly limit reached');
        ...
        if (walletbalance !== null && walletbalance > 0 && walletbalance >= parseInt(amount)) {
            console.log('Wallet sufficient, proceeding...');
            ...
            let affiliate_rate = 0;
            let affiliate_amount = 0;
            let affiliate_wallet_balance = 0;
            ...
            if (totalRechargeOfaMonth == 0 && wallet == 'Prime' && user_type != 'Prime' && check_assured == '0') {
                prime_rate = '5.00';
                prime_amount = amount * prime_rate / 100;
                console.log('Non-prime first recharge prime discount:', prime_amount);
            ...
            if ((!type.includes('electricity') || !type.includes('fasttag')) && check_assured == '0') {
                console.log('Checking discount for operator:', operatorId, 'type:', type);
                let discountAmount = await this.db.rechargeServiceDiscount.getOperatorCode(operatorId, type);
                console.log('Discount found:', discountAmount ? 'Yes' : 'No');
                ...
                if (discountAmount != null && Object.keys(discountAmount).length > 0) {
                    if (wallet == 'Cashback' && cashback_wallet_balance > 0) {
                        ...
                        console.log('Cashback rate:', cashback_rate);
                        ...
                        if (cashback_wallet_balance >= cashback_amount) {
                            cashback_amount = cashback_amount;
                            d_amount = d_amount - cashback_amount;
                            console.log('Cashback applied:', cashback_amount, 'New amount:', d_amount);
                        } else {
                            cashback_amount = 0;
                        }
                    }
                    ...
                    console.log('No discount setup found');
            ...
            if (totalDayRecharge <= 5 && rechargePermission === 0) {
                let getAllPanel = await this.db.panel.getAllData();
                console.log('Panels available:', getAllPanel.length);

                let flag = '';
                let group_transaction_no = utility.generateUniqueNumeric(7);
                
                for (const panel of getAllPanel) {
                    console.log('Trying panel:', panel.service_name);
                    ...
                    if (walletBalanceCheck === null || Number(walletBalanceCheck) <= 0 || Number(walletBalanceCheck) < parseInt(amount)) {
                        console.log('Wallet insufficient during panel processing');
                    ...
                    try {
                        const reqParam = {
                            userId, 'type': 'Debit', 'tran_for': 'Recharge', amount, ipAddress, ConsumerNumber, operatorId, d_amount, service_rate, service_amount,
                        ...
                        console.log('Recharge request result:', orderData.status, orderData.recharge_status);
                        ...
                        if (orderData.status == true && orderData.recharge_status == 'success') {
                            ...
                            if (typeof rechargeUtility[excutable_fun] === 'function') {
                                ...
                                console.log('Operator code for panel:', operator_code, 'Circle ID:', circleId);

                                console.log('Calling external API:', excutable_fun);
                                ...
                                console.log('API Response:', { 
                                    ...
                                    message: response.message, 
                                    refrenceID: response.refrenceID 
                                });
                        ...
                        console.log('Panel error:', response ? response.message : 'No response');
                        
                        if (response != null && response.status == 'FAILURE') {
                            console.log('Processing refund for failed recharge');
                            ...
                            const reqParam = {
                                userId, 'type': 'Credit', 'tran_for': 'Refund', amount, ipAddress, ConsumerNumber, operatorId, d_amount, service_rate, service_amount,
    ...
    console.log('Unexpected flow reached');
    return res.status(400).json(utility.DataEncrypt(JSON.stringify({ status: 400, message: 'Bad request', data: [] })));
}
