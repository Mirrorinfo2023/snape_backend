...
class Notification {
  ...
  async addNotification(fileName, req, res) {
    ...
    try {
      ...
      const newNotification = await this.db.notification.insertData(notificationData, {
        transaction: t
      });
  ...
  async getNotification(req, res) {
    ...
    try {
      ...
      const getNotificationData = await this.db.notification.getData(whereCondition);
      ...
      const report = {
        ...
        totalCount: await this.db.notification.count({ where: whereCondition }, 'id'),
        totalSuccessFcm: await this.db.notification.count({ where: { ...whereCondition, status: 1 } }),
        totalReceivedFcm: await this.db.notification.count({ where: { ...whereCondition, status: 0 } }),
      }
