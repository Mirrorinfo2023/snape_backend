  ...
  async getEbookReport(req, res) {
    ...
    try {
      ...
      const report = {
        ...
        totalApproveEbook: await this.db.ebookList.count({ where: { status: `1` } }),
        totalDeactivatedEbook: await this.db.ebookList.count({ where: { status: `0` } }),
      }

      let eventDetails = [];
      let result = await this.db.ebookList.getAllData(whereCondition);
